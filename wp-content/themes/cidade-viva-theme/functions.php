...
function loopPost($atts, $content = null) {
	...
	$catId = get_term_by('name', $category, 'category')->term_id;
	?>
        <?php while ($wp_query->have_posts()) : $wp_query->the_post(); 
			if($catId ==  get_term_by('name', 'blog', 'category')->term_id) {
		?>
 		...
 		<?php    } 
			else if($catId ==  (get_term_by('name', 'revista', 'category')->term_id || get_term_by('name', 'coluna no jornal', 'category')->term_id)) { ?>
    		<link rel="stylesheet" type="text/css" href="<?php bloginfo( 'template_url' ) ?>/css/revista-jornal.css" />  
